...
public class PurchaseTest {
	...
	public void purchase() throws InterruptedException {
		String 	confirmationPurchaseMsg = DataTestReader.getConfirmationPurchaseMsg("./src/test/resources/", "testData.xlsx","purchase");
		...
		for(int i=0; i<validCredentials.size(); i++) {
			...
			cartProductsPage.clickCheckOutButton();
			checkoutInfoPage = new CheckOutInfoPage(driver);
			checkoutInfoPage.setFirstName();
			checkoutInfoPage.setLastName();
			checkoutInfoPage.setPostalCode();
			checkoutInfoPage.clickContinueButton();
			overviewPage = new OverviewPage(driver);
			overviewPage.clickFinishPurchaseButton();
			confirmationPage = new ConfirmationPage(driver);
			Assert.assertEquals(confirmationPurchaseMsg, confirmationPage.getConfirmationMessage());
		}	
	}
